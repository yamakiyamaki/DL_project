...
class FaceSphereDataset(Dataset):
    def __init__(
        ...
        root_dir,
        split="train",
        transforms_face=None,
        transforms_sphere=None,
        mask_path="./data/SphereMask.jpg",
    ):
        """
        Args:
        ...
        if not self.transforms_face:
            self.transforms_face = A.Compose([ToTensorV2()])

        self.transforms_sphere = transforms_sphere
        if not self.transforms_sphere:
            self.transforms_sphere = A.Compose([ToTensorV2()])
        ...
        try:
            ...
            self.maskTensor = (
                self.transforms_sphere(image=self.mask_3d)["image"].int().float()
            )
